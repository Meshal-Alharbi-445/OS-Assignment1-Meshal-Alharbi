...
long wait = (this.finishTime  -  this.creationTime) - this.burstTime;
...
static List<Process> completedProcesses = new ArrayList<Process>();
public static void main(String[] args) {
// ⚠️ IMPORTANT: Put your student ID here to seed the random number generator
// This makes your output unique to you - DO NOT forget to change this!
int studentID = 445050105;  // ← CHANGE THIS TO YOUR ACTUAL STUDENT ID
...
// Create a new process object with a unique name, burst time,defined time quantum,and priority
...
while (!processQueue.isEmpty()) {
        // Increment counter when switching to a new process execution
        contextSwitchCount++;
...
// Record finish time and add to table list for the last process
process.setFinishTime(System.currentTimeMillis());
completedProcesses.add(process);
       }
   } else {
       // Record finish time and add to table list for finished processes
       process.setFinishTime(System.currentTimeMillis());
       completedProcesses.add(process);
   }
}
...
System.out.printf(Colors.BOLD + Colors.CYAN + "│ %-13s │ %-13s │ %-13s │ %-13s │\n" + Colors.RESET,
                  "Process Name", "Burst Time", "Waiting Time", "Priority");
System.out.println(Colors.BOLD + Colors.CYAN + "├───────────────┼───────────────┼───────────────┼───────────────┤" + Colors.RESET);

for (Process p : completedProcesses) {
    System.out.printf(Colors.BOLD + Colors.CYAN + "│ " + Colors.RESET + "%-13s " +
                      Colors.BOLD + Colors.CYAN + "│ " + Colors.RESET + "%-13s " +
                      Colors.BOLD + Colors.CYAN + "│ " + Colors.RESET + "%-13s " +
                      Colors.BOLD + Colors.CYAN + "│ " + Colors.RESET + "%-13s " +
